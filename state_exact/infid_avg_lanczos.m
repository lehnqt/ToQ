function [iF,iG,overlap]=infid_avg_lanczos(H0,Hu,Hc,unc_tot,psi0,psi_tg,P,time_grid,c)
numK=25;
...
for j=1:bin_num
    ...
    psi_fw{j+1}=expvcpu(dt,H_tot,psi_fw{j},numK);
    for k=1:ctrl_num
...
%         D{k,j}=-1i*Delta_t*C1+(Delta_t^2/2)*C2+1i*(Delta_t^3/6)*C3;
    ...
    psi_bw{j+1}=expvcpu(-dt,H_tot,psi_bw{j},numK);
end
